...
import { Baby, BookOpen, ClipboardCheck } from 'lucide-react';
...
export default function BeginnerProfilePage() {
  const { id } = useParams<{ id: string }>();
  const [beginner, setBeginner] = useState<any>(null);
  ...
  useEffect(() => {
    if (!id) return;
    const load = async () => {
      // beginners/recitation_log(beginner_id) خارج الأنواع المولّدة — نفس نمط بقية الصفحات
      // نجلب الحلقة باستعلام منفصل (نفس نمط ملف المرافِقة) بدل الاعتماد على الـ join المضمّن
      const [bRes, rRes, aRes] = await Promise.all([
        (supabase as any).from('beginners').select('*').eq('id', id).single(),
        (supabase as any).from('recitation_log').select('*').eq('beginner_id', id).eq('is_deleted', false).order('date', { ascending: false }).limit(50),
        (supabase as any).from('attendance').select('*').eq('beginner_id', id).order('date', { ascending: false }).limit(50),
      ]);
      let beg = bRes.data;
      if (beg?.circle_id) {
        const { data: circle } = await supabase.from('circles').select('circle_name, branches(branch_name)').eq('id', beg.circle_id).single();
        beg = { ...beg, circles: circle };
      }
      setBeginner(beg);
  ...
  if (!beginner) return <div className="text-center py-12 text-destructive">المبتدئة غير موجودة</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-2xl font-display text-foreground">{beginner.full_name}</h1>
          <div className="flex flex-wrap gap-2 mt-1">
            <Badge variant="outline">مبتدئة</Badge>
            {beginner.admission_status && <Badge variant="outline">{statusLabels[beginner.admission_status] ?? beginner.admission_status}</Badge>}
            {beginner.circles && <Badge variant="secondary">{beginner.circles.circle_name}</Badge>}
            {beginner.circles?.branches && <Badge variant="secondary">{beginner.circles.branches.branch_name}</Badge>}
          </div>
        </div>
      </div>
        ...
        <TabsContent value="info">
          <Card>
            <CardContent className="pt-4 grid grid-cols-2 gap-4 text-sm">
              {[
                ['رقم الهوية', beginner.national_id],
                ['الهاتف', beginner.phone],
                ['البريد', beginner.email],
                ['هاتف ولي الأمر', beginner.guardian_phone],
                ['الجنسية', beginner.nationality],
                ['المؤهل', beginner.qualification],
                ['من سورة', beginner.from_surah],
                ['إلى سورة', beginner.to_surah],
                ['تاريخ التسجيل', beginner.registration_date],
                ['معها مرافقون', beginner.has_companions == null ? null : (beginner.has_companions ? 'نعم' : 'لا')],
                ['ملاحظات', beginner.notes],
              ].map(([label, value]) => (
